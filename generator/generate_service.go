...
func NewGenerateService(name, transport, pbPath, pbImportPath string, sMiddleware, eMiddleware bool, methods []string) Gen {
	i := &GenerateService{
		name:          name,
...
func (g *GenerateService) Generate() (err error) {
	...
	tp := NewGenerateTransport(g.name, g.transport, g.pbPath, g.pbImportPath, g.methods)
	err = tp.Generate()
	if err != nil {
		return err
	}
